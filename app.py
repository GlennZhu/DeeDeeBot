...
STOCK_TRIGGER_LABELS = {
    ...
    "entry_rsi_extreme_oversold": "Entry: Extreme RSI Oversold",
    ...
    "weekly_macd_bullish_divergence": "Bullish Weekly MACD Divergence",
    "weekly_macd_bearish_divergence": "Risk: Bearish Weekly MACD Divergence",
...
STOCK_TRIGGER_COLORS = {
    ...
    "entry_rsi_extreme_oversold": "#1f6feb",
    ...
    "weekly_macd_bullish_divergence": "#1f6feb",
    "weekly_macd_bearish_divergence": "#b26a00",
...
def _dashboard_data_dependencies() -> list[Path]:
    metric_series_paths = [RAW_DATA_DIR / f"{metric_key}.csv" for metric_key in METRIC_ORDER]
    return [
        DERIVED_DATA_DIR / "signals_latest.csv",
...
def _compute_dashboard_data_signature() -> tuple[tuple[str, int, int], ...]:
    signature: list[tuple[str, int, int]] = []
    for path in _dashboard_data_dependencies():
        try:
            stats = path.stat()
            signature.append((str(path), int(stats.st_mtime_ns), int(stats.st_size)))
        except OSError:
            signature.append((str(path), -1, -1))
    return tuple(signature)


def _refresh_cache_for_latest_dashboard_data() -> None:
    current_signature = _compute_dashboard_data_signature()
    previous_signature = st.session_state.get(APP_DATA_SIGNATURE_SESSION_KEY)
    if previous_signature != current_signature:
        st.cache_data.clear()
        st.session_state[APP_DATA_SIGNATURE_SESSION_KEY] = current_signature
